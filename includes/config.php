    ...
    try {
        ...
        if ($mm_row && $mm_row['setting_value'] === '1') {
            ...
            if ($user_role !== 'super_admin') {
                ...
                ?><!DOCTYPE html><html lang="<?php echo __('lang_code'); ?>" dir="<?php echo __('dir'); ?>"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title><?php echo h($maintenance_title); ?> — <?php echo h(SITE_NAME); ?></title><style>body{font-family:<?php echo __('dir') === 'rtl' ? "'Cairo',sans-serif" : "'Inter',sans-serif"; ?>;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#0f172a;color:#f1f5f9;text-align:center;padding:1rem}.mm-card{max-width:480px;padding:2rem}.mm-icon{font-size:3rem;margin-bottom:1rem}h1{font-size:1.5rem;margin-bottom:0.75rem}p{color:#94a3b8;line-height:1.6;font-size:0.9rem}</style></head><body><div class="mm-card"><div class="mm-icon">🔧</div><h1><?php echo h($maintenance_title); ?></h1><p><?php echo h($maintenance_msg); ?></p></div></body></html><?php
                exit();
            }
        }
    } catch (Exception $e) {
