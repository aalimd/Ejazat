...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_department'])) {
    ...
    if (empty($name_ar) || empty($name_en)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE departments SET name_ar = ?, name_en = ? WHERE id = ?");
            if ($stmt->execute([$name_ar, $name_en, $id])) {
...
$departments = $pdo->query("SELECT d.*, (SELECT COUNT(*) FROM employees WHERE department_id = d.id) as emp_count FROM departments d ORDER BY d.name_ar ASC")->fetchAll();
